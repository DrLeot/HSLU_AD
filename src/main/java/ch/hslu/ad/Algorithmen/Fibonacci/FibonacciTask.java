...
public final class FibonacciTask extends RecursiveTask<Long> {
    ...
    protected Long compute() {
        if (n <= 20) {
            ...
            FibonacciTask previous = new FibonacciTask(this.n - 1);
            previous.fork();

            FibonacciTask prePrevious = new FibonacciTask(this.n - 2);

            final Long sumPrePrevious = prePrevious.invoke();
            final Long sumPrevious = previous.join();

            return sumPrePrevious + sumPrevious;
        }
    }
}
